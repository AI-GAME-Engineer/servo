...
impl CalcNode {
    ...
    /// value.
    pub fn to_length_or_percentage(self) -> Result<CalcLengthOrPercentage, ()> {
        let mut ret = CalcLengthOrPercentage::default();
        self.into_length_or_percentage(&mut ret, 1.0)?;
        Ok(ret)
    }
        ...
        -> Result<(), ()>
    {
        match self {
            ...
            CalcNode::Length(l) => {
                match l {
                    NoCalcLength::Absolute(abs) => {
                        // FIXME(emilio): Can we do better the factor dance
                        // here without complicating the code too much?
                        ret.absolute = Some(
                            ret.absolute.unwrap_or(Au(0)) +
                            Au::from_f32_px(Au::from(abs).to_f32_px() * factor)
                        );
                    }
            ...
            CalcNode::Sub(a, b) => {
                a.into_length_or_percentage(ret, factor)?;
                b.into_length_or_percentage(ret, factor * -1.0)?;
            }
            CalcNode::Sum(a, b) => {
                a.into_length_or_percentage(ret, factor)?;
                b.into_length_or_percentage(ret, factor)?;
            }
            CalcNode::Mul(a, b) => {
                let new_factor = b.to_number()?;
                a.into_length_or_percentage(ret, factor * new_factor)?;
            }
            CalcNode::Div(a, b) => {
                let new_factor = b.to_number()?;
                if new_factor == 0. {
    ...
    pub fn to_angle(self) -> Result<Angle, ()> {
        Ok(match self {
            CalcNode::Angle(angle) => angle,
            CalcNode::Sub(a, b) => {
                let lhs = a.to_angle()?;
                let rhs = b.to_angle()?;
    ...
    pub fn to_number(self) -> Result<CSSFloat, ()> {
        Ok(match self {
            CalcNode::Number(n) => n,
            CalcNode::Sum(a, b) => {
